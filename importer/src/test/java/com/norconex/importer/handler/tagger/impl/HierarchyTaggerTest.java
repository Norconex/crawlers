...
class HierarchyTaggerTest {

    @Test
    void testWriteRead() {
        var tagger = new HierarchyTagger();
    ...
    void testMultiCharSeparator() {
        var meta = createDefaultTestMetadata(
                "//vegetable//potato//sweet");
        var tagger = createDefaultTagger("//", "!");
    ...
    void testRegexSeparatorWithToSep() {
        ...
        var tagger = createDefaultTagger("/\\d/", "!");
    ...
    void testRegexSeparatorWithoutToSep() {
        var meta = createDefaultTestMetadata(
                "/1/vegetable/2/potato/3//4/sweet");
        var tagger = createDefaultTagger("/\\d/", null);
        tagger.getHierarchyDetails().get(0).setRegex(true);
        Assertions.assertArrayEquals(new String[] {
                "/1/vegetable",
    ...
    private void tagAndAssert(
        ...
        var meta = createDefaultTestMetadata();
        var tagger = createDefaultTagger(fromSep, toSep);
