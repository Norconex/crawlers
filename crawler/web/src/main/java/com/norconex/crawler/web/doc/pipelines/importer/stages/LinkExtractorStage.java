...
public class LinkExtractorStage extends AbstractImporterStage {
    ...
    protected boolean executeStage(ImporterPipelineContext context) { //NOSONAR
        ...
        var maxDepth = Web.config(ctx.getCrawlSession().getCrawlContext())
                .getMaxDepth();
        if (maxDepth != -1
    ...
    private Set<Link> extractLinks(WebImporterPipelineContext ctx) {
        ...
        var is = ctx.getDocContext().getDoc().getInputStream();
    ...
    private String queueURL(
        ...
        if (uniqueExtractedURLs.add(link.getUrl())) {
            ...
            ctx.getCrawlSession().getCrawlContext()
                    .getDocPipelines()
                    .getQueuePipeline()
                    .accept(new QueuePipelineContext(ctx.getCrawlSession(),
                            newURL));
    ...
    private static class UniqueDocLinks {
        final Set<String> extracted = new ListOrderedSet<>();
        final Set<String> inScope = new ListOrderedSet<>();
        final Set<String> outScope = new ListOrderedSet<>();
    }
}
