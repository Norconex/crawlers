 ...
 */
@Slf4j
public class ResetDocLedgerTask extends GridInitializedCrawlerTask {
    ...
    protected void runWithInitializedCrawler(Crawler crawler, String arg) {
        var globalCache = crawler.getGridSystem().getGlobalCache();
    ...
    private void prepareForCrawl(Crawler crawler) {
        ...
        var globalCache = grid.getGlobalCache();
        var ledger = crawler.getDocProcessingLedger();

        if (Boolean.parseBoolean(
                globalCache.get(DocProcessingLedger.KEY_RESUMING))) {
