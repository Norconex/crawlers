...
public class IgniteGridStorage implements GridStorage {
    ...
    private static final String GLOBAL_CACHE = "global-cache";
    private static final String STORE_TYPES_KEY =
            "ignite-object-and-store-types";

    private final IgniteGrid igniteGrid;

    private IgniteGridMap<StoreSpecs> cacheObjectAndStoreTypes;

    @JsonIgnore
    @Override
    public <T> GridMap<T> getMap(
            @NonNull String storeName, @NonNull Class<? extends T> objectType) {
        cacheStoreSpecs(storeName, objectType, GridMap.class);
        return new IgniteGridMap<>(
                igniteGrid.api(), storeName, objectType);
    }
    ...
    public void clean() {
        ...
        var catalog = igniteGrid.api().catalog();
        igniteGrid
                .api()
                .tables()
                .tables()
    ...
    GridStore<?> concreteStore(
        ...
        Class<? extends GridStore> concreteType = IgniteGridMap.class;
    ...
    private synchronized IgniteGridMap<StoreSpecs> storeSpecs() {
        if (cacheObjectAndStoreTypes == null) {
            cacheObjectAndStoreTypes = new IgniteGridMap<>(
                    igniteGrid.api(),
                    STORE_TYPES_KEY,
                    StoreSpecs.class);
        }
