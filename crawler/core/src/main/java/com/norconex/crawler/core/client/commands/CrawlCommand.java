...
import com.norconex.crawler.core.tasks.CrawlTask;
import com.norconex.crawler.core.tasks.PrepareDocLedgerTask;
import com.norconex.crawler.core.tasks.QueueStartReferencesTask;
...
public class CrawlCommand implements Command {
    ...
    @Override
    public void execute(CrawlerClient crawlerClient) {
        prepareDocLedger(crawlerClient);
        queueStartReferences(crawlerClient);
        crawlDocs(crawlerClient);
        //        handleOrphanDocs(crawlerClient);
    }

    // On 1 node, block
    private void prepareDocLedger(CrawlerClient crawlerClient) {
    ...
    // On 1 node, maybe block
    private void queueStartReferences(CrawlerClient crawlerClient) {
    ...
    // On all nodes, block
    private void crawlDocs(CrawlerClient crawlerClient) {
        LOG.info("Crawling...");
