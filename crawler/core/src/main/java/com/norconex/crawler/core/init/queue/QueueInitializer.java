...
@Slf4j
@EqualsAndHashCode
public class QueueInitializer implements Predicate<CrawlerContext> {
    ...
    public boolean test(CrawlerContext crawlerContext) {
        ...
        var callback = (Consumer<QueueInitContext>) ctx -> {
            var cnt = 0;
    ...
    private void initializeQueueAsync(
            Consumer<QueueInitContext> callback, QueueInitContext ctx) {
