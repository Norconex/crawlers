 ...
 */
public interface DataStoreEngine2 extends Closeable {

    // atomic transaction -- multiple operations done as a single operation
    // locked: boolean -- whether only 1 at at time or several can .
    // block: boolean -- whether other clients wait until finished
    //                       regardless whether they also execute or not
    // singleton: boolean -- whether only 1 executes
    ...
    //     * Default implementation does not support transaction.
    //     * @param <T> the return type
    //     * @param callable code executed involving store transactions.
    //     * @return any value as per the callable passed
    //     * @throws DataStoreException
    //     */
    //    default <T> T atomically(
    //            @NonNull FailableCallable<T, Exception> callable,
    //            boolean block)
    //            throws DataStoreException {
    //        try {
    //            // Transaction would be started here
    //            return callable.call();
    //            // Transaction would be ended here
    //
    //        } catch (Exception e) {
    ...
    //TODO rename gridExecutor?
    //TODO rename transactionExecutor()
    DataStoreGridExecutor gridExecutor() throws DataStoreException;
    ...
    /**
     * Opens (or creates as needed) a key-value store of the given name to
     * store and retrieve objects of the given type.
     * @param <T> class type of the object kept in the store
     * @param name store name
     * @param type a class for type of the object kept in the store
     * @return a data store
     */
    <T> KeyValueStore<T> openKeyValueStore(String name,
            Class<? extends T> type);

    /**
     * Opens (or creates as needed) a queue store of the given name to
     * store and retrieve objects of the given type.
     * @param <T> class type of the object kept in the store
     * @param name store name
     * @param type a class for type of the object kept in the store
     * @return a data store
     */
    <T> QueueStore<T> openQueueStore(String name, Class<? extends T> type);
