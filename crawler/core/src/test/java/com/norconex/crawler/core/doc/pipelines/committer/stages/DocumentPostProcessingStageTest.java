...
/**
 * Tests for {@link DocumentPostProcessingStage}.
 */
@Timeout(30)
class DocumentPostProcessingStageTest {
    ...
    void noConsumers_returnsTrue() {
        ...
        assertThat(new DocumentPostProcessingStage().test(ctx)).isTrue();
    }
    ...
    void noConsumers_noEventsfired() {
        ...
        new DocumentPostProcessingStage().test(ctx);

        verify(session, never()).fire(any());
    }

    // -----------------------------------------------------------------
    // With consumers configured
    // -----------------------------------------------------------------
    ...
    void withTwoConsumers_bothCalledAndTwoEventsFired() {
        ...
        config.setPostImportConsumers(List.of(c1, c2));
        ...
        verify(session, times(2)).fire(any());
    }
    ...
    void consumer_receivesCorrectFetcher() {
        ...
        assertThat(receivedFetcher[0]).isSameAs(fetcher);
    }
}
