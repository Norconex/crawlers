...
public class AppClusterExtension
        implements BeforeAllCallback, AfterAllCallback,
        BeforeEachCallback, AfterEachCallback, ParameterResolver {
    private static final ExtensionContext.Namespace NAMESPACE =
            ExtensionContext.Namespace.create(AppClusterExtension.class);

    // Helper to find annotation on method or class
    private Optional<AppCluster> findAnnotation(ExtensionContext ctx) {
        // Check method first
        if (ctx.getElement().isPresent()) {
            var ann =
                    ctx.getElement().get().getAnnotation(AppCluster.class);
            if (ann != null) {
        ...
        if (ctx.getTestClass().isPresent()) {
            var ann =
                    ctx.getTestClass().get().getAnnotation(AppCluster.class);
            if (ann != null) {
                return Optional.of(ann);
            }
        }
    ...
    private List<GenericContainer<?>> startContainers(AppCluster config) {
    ...
    public void beforeAll(ExtensionContext context) {
        ...
        var ann = context.getTestClass()
                .map(c -> c.getAnnotation(AppCluster.class)).orElse(null);
        if (ann != null) {
    ...
    public void beforeEach(ExtensionContext context) {
        ...
        var ann = context.getElement()
                .map(e -> e.getAnnotation(AppCluster.class)).orElse(null);
        if (ann != null &&
                context.getTestClass()
                        .map(c -> c.getAnnotation(AppCluster.class))
                        .isEmpty()) {
            var containers = startContainers(ann);
            context.getStore(NAMESPACE).put(storeKey(context), containers);
        }
        ...
        if (containers == null) {
            throw new IllegalStateException(
                    "No containers found for injection. " +
                            "Did you forget to annotate the test class or method with @AppCluster?");
        }
        return containers;
    }
}
