    ...
    public void accept(CrawlSession session, Class<? extends Command> cmdCls) {
        ...
        cacheManager.exportCaches(serialCache -> {
            ...
            for (var entry : serialCache) {
                // Cache keys can legitimately be null (e.g., default queue).
                // Jackson does not allow null map keys during (YAML/JSON)
                // serialization, so normalize keys for test output.
                var key = entry.getKey() == null ? "<null>"
                        : String.valueOf(entry.getKey());
                cacheMap.put(key, entry.getJson());
            }
        });
    }
}
