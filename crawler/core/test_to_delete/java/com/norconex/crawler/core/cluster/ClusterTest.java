    ...
    void testStopResumeClusterExecution(int numNodes) throws Exception {
        ...
        try (var harness = new CrawlTestHarness(instrument)) {
            ...
            harness.waitFor(Duration.ofSeconds(60))
                    .allNodesToHaveFired(CrawlerEvent.CRAWLER_CRAWL_BEGIN);
            // Then ensure at least one node has started importing documents.
            // Work distribution in a cluster isn't deterministic, and waiting
            // for every node to import can burn the overall test timeout.
            harness.waitFor(Duration.ofSeconds(30))
                    .anyNodeToHaveFired(CrawlerEvent.DOCUMENT_IMPORTED);
            ...
            var coordinatorOutput = secondResult.getNodeOutputs().values()
                    .stream()
                    .filter(output -> !output.getCaches().isEmpty()
                            && output.getCaches()
