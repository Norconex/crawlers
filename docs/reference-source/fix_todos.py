...
lookup = {
    ...
    # WebCrawlConfig.mdx - shared with CrawlConfig
    ('WebCrawlConfig.mdx', 'id'): 'Unique identifier for the crawler, used in logging and cluster coordination.',
    ('WebCrawlConfig.mdx', 'numThreads'): 'Number of threads used to crawl documents in parallel.',
    ('WebCrawlConfig.mdx', 'maxDepth'): 'Maximum crawl depth from start references. A depth of 0 means only the start references themselves are processed.',
    ('WebCrawlConfig.mdx', 'maxDocuments'): 'Maximum total number of documents to process across all crawl runs combined.',
    ('WebCrawlConfig.mdx', 'maxDocumentsPerRun'): 'Maximum number of documents to process in a single crawl run.',
    ('WebCrawlConfig.mdx', 'maxRunDocuments'): 'Maximum number of documents to process in a single crawl run.',
    ('WebCrawlConfig.mdx', 'maxCrawlDuration'): 'Maximum duration a crawl run is allowed to run before stopping gracefully.',
    ('WebCrawlConfig.mdx', 'maxQueueBatchSize'): 'Number of references dequeued and dispatched to crawl threads at a time.',
    ('WebCrawlConfig.mdx', 'maxStreamCacheSize'): 'Maximum memory a single document stream may use before spilling to disk.',
    ('WebCrawlConfig.mdx', 'maxStreamCachePoolSize'): 'Maximum total memory used across all in-memory document stream caches.',
    ('WebCrawlConfig.mdx', 'minProgressLoggingInterval'): 'Minimum interval between crawl progress log entries.',
    ('WebCrawlConfig.mdx', 'idleTimeout'): 'How long the crawler waits with no activity before stopping. Helps detect stalled crawls.',
    ('WebCrawlConfig.mdx', 'deferredShutdownDuration'): 'Grace period given to in-flight work when a shutdown signal is received.',
    ('WebCrawlConfig.mdx', 'importer'): 'The importer pipeline configuration for document parsing and metadata enrichment.',
    ('WebCrawlConfig.mdx', 'fetchers'): 'One or more fetchers responsible for retrieving document content from the source.',
    ('WebCrawlConfig.mdx', 'fetchersMaxRetries'): 'Maximum number of retry attempts when a fetcher fails to retrieve a document.',
    ('WebCrawlConfig.mdx', 'fetchersRetryDelay'): 'Time to wait between fetcher retry attempts.',
    ('WebCrawlConfig.mdx', 'committers'): 'One or more committers that send processed documents to target systems (e.g., search engines, databases).',
    ('WebCrawlConfig.mdx', 'eventListeners'): 'Handlers invoked in response to crawl lifecycle events such as document fetched, rejected, or committed.',
    ('WebCrawlConfig.mdx', 'documentFilters'): 'Filters that determine which documents are included in or excluded from processing.',
    ('WebCrawlConfig.mdx', 'metadataFilters'): 'Filters applied to document metadata to include or exclude documents based on field values.',
    ('WebCrawlConfig.mdx', 'metadataChecksummer'): 'Computes a checksum from document metadata fields, used for change detection and deduplication.',
    ('WebCrawlConfig.mdx', 'documentChecksummer'): 'Computes a checksum from document content, used for change detection and deduplication.',
    ('WebCrawlConfig.mdx', 'metadataDeduplicate'): 'When enabled, documents with a previously seen metadata checksum are skipped.',
    ('WebCrawlConfig.mdx', 'documentDeduplicate'): 'When enabled, documents with a previously seen content checksum are skipped.',
    ('WebCrawlConfig.mdx', 'metadataFetchSupport'): 'Controls whether a metadata-only (e.g., HTTP HEAD) fetch is attempted before a full document fetch.',
    ('WebCrawlConfig.mdx', 'documentFetchSupport'): 'Controls whether document content is fetched. Disable for metadata-only crawling.',
    ('WebCrawlConfig.mdx', 'cluster'): 'Configuration for distributed, multi-node crawling. Leave unset for single-node operation.',
    # WebCrawlConfig.mdx only
    ('WebCrawlConfig.mdx', 'delayResolver'): 'Determines the politeness delay between consecutive requests to the same host.',
    ('WebCrawlConfig.mdx', 'linkExtractors'): 'Extractors that discover outbound links within crawled documents.',
    ('WebCrawlConfig.mdx', 'urlNormalizers'): 'Normalizers that standardize URL formats before they are stored or followed.',
    ('WebCrawlConfig.mdx', 'urlScopeResolver'): 'Determines whether a discovered URL falls within the crawl scope.',
    ('WebCrawlConfig.mdx', 'recrawlableResolver'): 'Determines whether a previously crawled document is eligible for re-crawling on this run.',
    ('WebCrawlConfig.mdx', 'sitemapLocator'): 'Locates sitemap files for crawled websites.',
    ('WebCrawlConfig.mdx', 'sitemapResolver'): 'Parses and processes sitemap files to extract crawl references.',
    ('WebCrawlConfig.mdx', 'robotsTxtProvider'): 'Provides robots.txt crawl rules for a domain.',
    ('WebCrawlConfig.mdx', 'robotsMetaProvider'): 'Extracts robot directives from HTML meta tags and HTTP response headers.',
    ('WebCrawlConfig.mdx', 'canonicalLinkDetector'): 'Detects canonical link relationships to avoid crawling duplicate pages.',
    ('WebCrawlConfig.mdx', 'keepReferencedLinks'): 'Which referenced links to keep as metadata on the crawled document (e.g., in-scope, out-of-scope, or all).',
    ('WebCrawlConfig.mdx', 'postImportLinks'): 'References discovered after the importer has processed the document.',
    ('WebCrawlConfig.mdx', 'postImportLinksKeep'): 'When enabled, links discovered after import are retained as document metadata.',
    ('WebCrawlConfig.mdx', 'startReferencesSitemaps'): 'Sitemap URLs used as additional sources of start references.',
    # ClusterConfig.mdx
