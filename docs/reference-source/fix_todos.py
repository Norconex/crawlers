...
lookup = {
    # CrawlerConfig.mdx
    ('CrawlerConfig.mdx', 'id'): 'Unique identifier for the crawler, used in logging and cluster coordination.',
    ('CrawlerConfig.mdx', 'numThreads'): 'Number of threads used to crawl documents in parallel.',
    ('CrawlerConfig.mdx', 'maxDepth'): 'Maximum crawl depth from start references. A depth of 0 means only the start references themselves are processed.',
    ('CrawlerConfig.mdx', 'maxDocuments'): 'Maximum number of documents to process in a single crawl run before stopping. If the queue still contains documents, the crawl session is paused and resumes on the next run.',
    ('CrawlerConfig.mdx', 'maxCrawlDuration'): 'Maximum duration a crawl run is allowed to run before stopping gracefully.',
    ('CrawlerConfig.mdx', 'maxQueueBatchSize'): 'Number of references dequeued and dispatched to crawl threads at a time.',
    ('CrawlerConfig.mdx', 'maxStreamCacheSize'): 'Maximum memory a single document stream may use before spilling to disk.',
    ('CrawlerConfig.mdx', 'maxStreamCachePoolSize'): 'Maximum total memory used across all in-memory document stream caches.',
    ('CrawlerConfig.mdx', 'minProgressLoggingInterval'): 'Minimum interval between crawl progress log entries.',
    ('CrawlerConfig.mdx', 'idleTimeout'): 'How long the crawler waits with no activity before stopping. Helps detect stalled crawls.',
    ('CrawlerConfig.mdx', 'deferredShutdownDuration'): 'Grace period given to in-flight work when a shutdown signal is received.',
    ('CrawlerConfig.mdx', 'importer'): 'The importer pipeline configuration for document parsing and metadata enrichment.',
    ('CrawlerConfig.mdx', 'fetchers'): 'One or more fetchers responsible for retrieving document content from the source.',
    ('CrawlerConfig.mdx', 'fetchersMaxRetries'): 'Maximum number of retry attempts when a fetcher fails to retrieve a document.',
    ('CrawlerConfig.mdx', 'fetchersRetryDelay'): 'Time to wait between fetcher retry attempts.',
    ('CrawlerConfig.mdx', 'committers'): 'One or more committers that send processed documents to target systems (e.g., search engines, databases).',
    ('CrawlerConfig.mdx', 'eventListeners'): 'Handlers invoked in response to crawl lifecycle events such as document fetched, rejected, or committed.',
    ('CrawlerConfig.mdx', 'documentFilters'): 'Filters that determine which documents are included in or excluded from processing.',
    ('CrawlerConfig.mdx', 'metadataFilters'): 'Filters applied to document metadata to include or exclude documents based on field values.',
    ('CrawlerConfig.mdx', 'metadataChecksummer'): 'Computes a checksum from document metadata fields, used for change detection and deduplication.',
    ('CrawlerConfig.mdx', 'documentChecksummer'): 'Computes a checksum from document content, used for change detection and deduplication.',
    ('CrawlerConfig.mdx', 'metadataDeduplicate'): 'When enabled, documents with a previously seen metadata checksum are skipped.',
    ('CrawlerConfig.mdx', 'documentDeduplicate'): 'When enabled, documents with a previously seen content checksum are skipped.',
    ('CrawlerConfig.mdx', 'metadataFetchSupport'): 'Controls whether a metadata-only (e.g., HTTP HEAD) fetch is attempted before a full document fetch.',
    ('CrawlerConfig.mdx', 'documentFetchSupport'): 'Controls whether document content is fetched. Disable for metadata-only crawling.',
    ('CrawlerConfig.mdx', 'cluster'): 'Configuration for distributed, multi-node crawling. Leave unset for single-node operation.',
    ('CrawlerConfig.mdx', 'spoiledReferenceStrategizer'): 'Strategy for handling references that repeatedly fail to be processed (spoiled references).',
    # WebCrawlerConfig.mdx - shared with CrawlerConfig
    ...
    ('WebCrawlerConfig.mdx', 'maxDocuments'): 'Maximum number of documents to process in a single crawl run before stopping. If the queue still contains documents, the crawl session is paused and resumes on the next run.',
